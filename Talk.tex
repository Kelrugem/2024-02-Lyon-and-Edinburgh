...
\begin{remark}[$\mathcal{F}$-connections]
...
\varphi \cdot p
\eas
for all $p \in \mathcal{T}$, $\phi \in \mathup{Sym}(\tau)$, and
for some $\varphi \in \mathup{Inner}\mleft(\tau_{\pi(p)}\mright)$.
\end{remark}
...
\draw (14.75,-4.2) node {\textcolor[rgb]{1,0,0}{$x$}};
\draw (15.5,-3.6) node {$)$};
